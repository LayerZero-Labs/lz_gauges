...
interface BalBoost:
    def totalSupplyVE() -> uint256: view
    def receivedBoosts(_user: address) -> Boost: view
...
@external
def __init__(_bal_token: address, _vault: address, _authorizerAdaptor: address, _bal_boost: address):
    """
...
def _checkpoint_rewards(_user: address, _total_supply: uint256, _claim: bool, _receiver: address):
    ...
    for i in range(MAX_REWARDS):
        ...
        # new... takes into account boosted values for BAL_TOKEN
        if token == BAL_TOKEN:
        ...
        if total_supply != 0:
            ...
            dI = 10**18 * (token_balance - self.reward_balances[token]) / total_supply
...
def adjusted_balance_of(_account: address) -> uint256:
    boost: Boost = BalBoost(BAL_BOOST).boosts(_account)

    next_expiry: uint256 = boost.expiry_data % 2 ** 128
    if next_expiry != 0 and next_expiry < block.timestamp:
        return 0

    adjusted_balance: int256 = 0
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    voting_balance: uint256 = self.adjusted_balance_of(addr)
    voting_total: uint256 = BalBoost(BAL_BOOST).totalSupplyVE()
